(function(_, glu, gldebug, window) {
  ...
  instances = [
    new Instance(
      function nullScale() { return [1.0, 1.0, 1.0]; },
      [0, 0, -45]
    ),
    new Instance(
      function staticUniformScale() { return [4.0, 4.0, 4.0]; },
      [-10, -10, -45]
    ),
    new Instance(
      function staticNonUnformScale() { return [ 0.5, 1.0, 10.0 ]; },
      [-10, 10, -45]
    ),
    new Instance(
      function dynamicUniformScale(elapsedTime) {
        var loopDuration = 3.0, 
        value = mix(1.0, 4.0, calcLerpFactor(elapsedTime, loopDuration));
        return [ value, value, value ];
      },
      [ 10, 10, -45]
    ),
    new Instance(
      function dynamicNonUniformScale(elapsedTime) {
        var xLoopDuration = 3.0,
        zLoopDuration = 5.0;
        
        return [
          mix(1.0, 5.0, calcLerpFactor(elapsedTime, xLoopDuration)),
          1.0,
  ...
  /**
   * based on glm::mix documentation:
   * http://glm.g-truc.net/api-0.9.4/a00129.html#ga3f64b3986efe205cf30300700667e761
   */
  function mix(x, y, a) {
    return x*(1.0 - a) + y*a;
  }

  function calcLerpFactor(time, duration) {
    var value = (time % duration) / duration;
    if (value > 0.5) {
      value = 1.0 - value;
    }
  ...
  function Instance(scaleFn, offset) {
    this.constructMatrix = function(elapsedTime) {
      var scale = scaleFn(elapsedTime);
